...
interface MatchDetailsModalProps {
  ...
  userID?: string; // to link to /messages
}
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="text-center p-6 bg-gray-50 rounded-lg">
            <div className={`text-4xl font-bold ${getMatchColor(overallScore)} mb-2`}>
              {overallScore}%
            </div>
                  ...
                  <div key={category.name} className="flex items-center gap-4 p-4 border rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`font-semibold ${getMatchColor(category.score)}`}>
                          {category.score}%
                        </span>
                      </div>
            ...
            {userID && (
              <Link
                href={`/messages?recipient=${encodeURIComponent(
                  userID,
                )}&recipientName=${encodeURIComponent(profileName || "")}`}
          ...
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-sm text-blue-800 space-y-1">
              ...
              {overallScore >= 60 && overallScore < 80 && (
                <p>• This is a good potential match. Focus on discussing areas where you align.</p>
              )}
              {overallScore >= 40 && overallScore < 60 && (
                <p>• This match has potential but may require more discussion.</p>
              )}
              {overallScore < 40 && (
                <p>• This match may be challenging. Consider if the differences are manageable for you.</p>
              )}
              {dealBreakers.length > 0 && (
                <p>• Discuss the considerations listed above before committing.</p>
              )}
